...
def query():
    ...
    for i in range(3):
        for row in table:
            ...
            if i == 0:
                ...
                if not exists:
                    add(row, mf_struct, ['cust', 'prod'], ['0_sum_quant', '1_sum_quant', '1_max_quant', '2_min_quant'])
                update(row, mf_struct, ['cust', 'prod'], [['0_sum_quant'], ['1_sum_quant', '1_max_quant'], ['2_min_quant']][i], "True") # update the rows in mf_struct corresponding to i=0 (aggregates over the standard SQL groups)
            else:
                update(row, mf_struct, ['cust', 'prod'], [['0_sum_quant'], ['1_sum_quant', '1_max_quant'], ['2_min_quant']][i], ["row['state']=='NY'", "row['quant'] < 1_max_quant"][i-1]) # update the rows in mf_struct corresponding to i!=0 (aggregates over the grouping variables)             

    
    output(mf_struct, ['cust', 'prod'], ['cust', 'prod', '0_sum_quant', '1_sum_quant']) # check mf_struct output
    ...
    if (16 != 0):
        ...
        for key, value in mf_struct.items():
            ...
            if (value['2_min_quant'] > 30):
                ...
                proj_d = {}
                for key, value in d.items():
                    if key in ['cust', 'prod', '0_sum_quant', '1_sum_quant']:
                        proj_d[key] = value
    ...
    for key1, dict in mf_struct.items():
        for key2 in list(dict.keys()):
            if key2 not in ['cust', 'prod', '0_sum_quant', '1_sum_quant']:
                del dict[key2]
