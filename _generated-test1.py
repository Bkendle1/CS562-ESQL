    ...
    for i in range(3):
        for row in table:
            ...
            if i == 0:
                ...
                update(row, mf_struct, ['cust', 'prod'], [['0_sum_quant'], ['1_sum_quant'], ['2_sum_quant']][i], ["row['state']=='PA' and row['quant']>=200", "row['state']=='CT' and row['quant']<=200"][i-1]) # update the rows in mf_struct corresponding to i!=0 (aggregates over the grouping variables)             

    
    output(mf_struct, ['cust', 'prod'], ['cust', 'prod', '0_sum_quant', '1_sum_quant', '2_sum_quant']) # check mf_struct output
    ...
    if (0 != 0):
        ...
        for key, value in mf_struct.items():
            ...
            if ():
                ...
                proj_d = {}
                for key, value in d.items():
                    if key in ['cust', 'prod', '0_sum_quant', '1_sum_quant', '2_sum_quant']:
                        proj_d[key] = value
